...
- (BOOL)makeFirstResponder:(NSResponder *)responder
{
	if (!textview && !textfield) {
		if (!textview && [responder isKindOfClass:[NSTextView class]])
			textview = (NSTextView *)responder;
		else if (!textfield && [responder isKindOfClass:[NSTextField class]])
			textfield = (NSTextField *)responder;

		if (textview || textfield)
			[self performSelector:@selector(selectNamePart)
					   withObject:nil
					   afterDelay:0
