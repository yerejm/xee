				...
				switch (chunkid) {
					...
					case 0x0406: // JPEG quality
					{
						...
						switch (quality) {
							...
							case 3:
							case 4:
								qualitystr = NSLocalizedString(@" (Low)", @"Low JPEG quality property value");
								break;
							case 5:
							case 6:
							case 7:
								qualitystr = NSLocalizedString(@" (Medium)", @"Medium JPEG quality property value");
								break;
							case 8:
							case 9:
								qualitystr = NSLocalizedString(@" (High)", @"High JPEG quality property value");
								break;
							case 10:
							case 11:
							case 12:
								qualitystr = NSLocalizedString(@" (Maximum)", @"Maximum JPEG quality property value");
								break;
							default:
								qualitystr = NSLocalizedString(@" (Unknown)", @"Unknown JPEG quality property value");
								break;
						}
						[props addObject:[XeePropertyItem itemWithLabel:
						...
						switch (type) {
							case 0:
								typestr = NSLocalizedString(@"Standard", @"Standard JPEG type property value");
								break;
							case 1:
								typestr = NSLocalizedString(@"Optimized", @"Optimized JPEG type property value");
								break;
							case 2:
							case 257:
								typestr = NSLocalizedString(@"Progressive", @"Progressive JPEG type property value");
								break;
							default:
								typestr = NSLocalizedString(@"Unknown", @"Unknown JPEG type property value");
								break;
						}
						[props addObject:[XeePropertyItem itemWithLabel:
...
//						value:[[[NSString alloc] initWithData:[handle readDataOfLength:chunklen] encoding:NSISOLatin1StringEncoding] autorelease]]];
		...
		@catch (id e) {
			NSLog(@"Error parsing Photoshop metadata: %@ %@", e, props);
		}
	}
	return self;
}
...
- (NSArray *)propertyArrayWithPhotoshopFirst:(BOOL)psfirst
{
	...
	XeePropertyItem *psitem = [XeePropertyItem itemWithLabel:
												   NSLocalizedString(@"Photoshop properties", @"Photoshop properties section title")
													   value:props
												  identifier:@"photoshop"];

	if (psfirst)
		[array addObject:psitem];

	if (exifprops)
		[array addObjectsFromArray:exifprops];
	if (xmpprops)
		[array addObjectsFromArray:xmpprops];
	if (iptcprops)
		[array addObjectsFromArray:iptcprops];
