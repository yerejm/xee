...
static inline float cube(float a)
{
	return a * a * a;
}
static inline uint8_t clamp8(int a)
{
	if (a < 0)
		return 0;
	else if (a > 255)
...
static inline uint16_t clamp16(int a)
{
	if (a < 0)
		return 0;
	else if (a > 65535)
...
{
	return [self initWithHandle:inhandle
						  width:framewidth
						 height:frameheight
						  depth:framedepth
					colourSpace:space
...
- (id)initWithHandle:(CSHandle *)inhandle width:(int)framewidth height:(int)frameheight
		 ...
		 colourSpace:(XeeRawColourSpace)space
			   flags:(XeeRawAlphaFlags)flags
		 bytesPerRow:(int)bytesperinputrow
{
	if (self = [super initWithHandle:inhandle]) {
		...
		if (precomp) {
			if (bitdepth == 8) {
				if (alphafirst)
					uncomposition = XeeRawUncomposeAlphaFirst8;
				else if (space == XeeLabRawColourSpace)
					uncomposition = XeeRawUncomposeAlphaLab8;
				else
					uncomposition = XeeRawUncomposeAlphaLast8;
			} else if (bitdepth == 16) {
				if (alphafirst)
					uncomposition = XeeRawUncomposeAlphaFirst16;
				else if (space == XeeLabRawColourSpace)
					uncomposition = XeeRawUncomposeAlphaLab16;
				else
					uncomposition = XeeRawUncomposeAlphaLast16;
			} else {
				[self release];
				return nil;
			}
		...
		if (!hasalpha) {
			modealphafirst = XeeAlphaNone;
			modealphalast = XeeAlphaNone;
		} else if (skipalpha) {
			modealphafirst = XeeAlphaNoneSkipFirst;
			modealphalast = XeeAlphaNoneSkipLast;
		} else if (premult) {
			modealphafirst = XeeAlphaPremultipliedFirst;
			modealphalast = XeeAlphaPremultipliedLast;
		} else {
			modealphafirst = XeeAlphaFirst;
			modealphalast = XeeAlphaLast;
		}
		...
		switch (space) {
			...
			case XeeCMYKRawColourSpace:
				if (bitdepth == 8 && !alphafirst) {
					type = XeeBitmapType(XeeRGBBitmap, 8, modealphafirst, 0);
					if (hasalpha)
						transformation = XeeRawCMYKAConversion8;
					else
						transformation = XeeRawCMYKConversion8;
					needsbuffer = YES;
				} else if (bitdepth == 16 && !alphafirst) {
					type = XeeBitmapType(XeeRGBBitmap, 16, modealphalast, 0);
					if (hasalpha)
						transformation = XeeRawCMYKAConversion16;
					else
						transformation = XeeRawCMYKConversion16;
					needsbuffer = YES;
				}
				channels = 4;
				break;
...
- (void)load
{
	...
	for (int row = 0; row < height; row++) {
		...
		if (buffer)
			...
			datarow = XeeImageDataRow(self, row);

		[handle readBytes:bytesperinputrow toBuffer:datarow];

		if (inbpr && inbpr != bytesperinputrow)
			[handle skipBytes:inbpr - bytesperinputrow];
		...
		switch (uncomposition) {
				...
				for (int x = 0; x < width; x++) {
					if (ptr[channels - 1])
						for (int i = 0; i < channels - 1; i++)
							ptr[i] = ((ptr[i] - 255 + ptr[channels - 1]) * 255) / ptr[channels - 1];
					ptr += channels;
				}
			} break;
				...
				for (int x = 0; x < width; x++) {
					if (ptr[0])
						for (int i = 1; i < channels; i++)
							ptr[i] = ((ptr[i] - 255 + ptr[0]) * 255) / ptr[0];
					ptr += channels;
				}
			} break;
			...
			case XeeRawUncomposeAlphaLab8: {
				...
				for (int x = 0; x < width; x++) {
					if (ptr[3])
						ptr[0] = ((ptr[0] - 255 + ptr[3]) * 255) / ptr[3];
					ptr += 4;
				}
			} break;
			...
			case XeeRawUncomposeAlphaLast16: {
				...
				for (int x = 0; x < width; x++) {
					if (ptr[channels - 1])
						for (int i = 0; i < channels - 1; i++)
							ptr[i] = ((uint32_t)(ptr[i] - 65535 + ptr[channels - 1]) * 65535) / ptr[channels - 1];
					ptr += channels;
				}
			} break;
			...
			case XeeRawUncomposeAlphaFirst16: {
				...
				for (int x = 0; x < width; x++) {
					if (ptr[0])
						for (int i = 0; i < channels - 1; i++)
							ptr[i] = ((uint32_t)(ptr[i] - 65535 + ptr[0]) * 65535) / ptr[0];
					ptr += channels;
				}
			} break;
			...
			case XeeRawUncomposeAlphaLab16: {
				...
				for (int x = 0; x < width; x++) {
					if (ptr[3])
						ptr[0] = ((uint32_t)(ptr[0] - 65535 + ptr[3]) * 65535) / ptr[3];
					ptr += 4;
				}
			} break;
		}
		...
		switch (transformation) {
			...
			case XeeRawFlipRGBAlpha32: {
				uint32_t *ptr = (uint32_t *)XeeImageDataRow(self, row);
				for (int x = 0; x < width; x++) {
					uint32_t a = ptr[0], b = ptr[1], c = ptr[2], d = ptr[3];
					*ptr++ = b;
					*ptr++ = c;
					*ptr++ = d;
					*ptr++ = a;
			...
			case XeeRawCMYKConversion16: {
				...
				for (int x = 0; x < width; x++) {
					...
					*rgb++ = (uint32_t)(k * c) / 65535;
					*rgb++ = (uint32_t)(k * m) / 65535;
					*rgb++ = (uint32_t)(k * y) / 65535;
				}
			} break;
