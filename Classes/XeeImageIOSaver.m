...
- (NSMutableDictionary *)properties
{
	...
	if (colorspace && back) {
		...
		if (col)
			[properties setObject:(id)col forKey:(NSString *)kCGImageDestinationBackgroundColor];
		CGColorRelease(col);
	}
...
- (id)initWithImage:(XeeImage *)img
{
	if (self = [super initWithImage:img]) {
		if ([image transparent]) {
			alpha = [XeeSLSwitch switchWithTitle:nil
										   label:
...
- (id)initWithImage:(XeeImage *)img
{
	if (self = [super initWithImage:img]) {
		...
		interlaced = [XeeSLSwitch switchWithTitle:nil
											label:
