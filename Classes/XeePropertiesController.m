...
- (void)awakeFromNib
{
	...
	[[NSNotificationCenter defaultCenter] addObserver:self
											 selector:@selector(frontImageDidChange:)
												 name:XeeFrontImageDidChangeNotification
											   object:nil];
	...
	if (delegate)
		[[NSNotificationCenter defaultCenter] postNotificationName:XeeFrontImageDidChangeNotification object:delegate];
}
...
- (void)toggleVisibility
{
	if ([infopanel isVisible]) {
		...
		XeeController *delegate = [maindelegate focusedController];
		if (delegate)
			[self setFullscreenMode:[delegate isFullscreen]];
	}
}
		...
		if ([identifier isEqual:@"label"])
			return [[[NSAttributedString alloc]
				initWithString:[item label]
					attributes:[item isSubSection] ? sectionattributes : labelattributes] autorelease];
		// return [item label];
...
- (NSString *)outlineView:(NSOutlineView *)view toolTipForCell:(NSCell *)cell rect:(NSRectPointer)rect
			  tableColumn:(NSTableColumn *)col
					 item:(XeePropertyItem *)item
			mouseLocation:(NSPoint)mouse
{
	if ([item isSubSection])
		return @"";
	if ([[col identifier] isEqual:@"label"])
		return [item label];
	else
...
- (void)restoreCollapsedStatusForArray:(NSArray *)array
{
	...
	while (item = [enumerator nextObject]) {
		...
		if ([value isKindOfClass:[NSArray class]]) {
			NSString *defname = [NSString stringWithFormat:@"propertyListCollapsed.%@", [item identifier]];
			if ([defaults boolForKey:defname])
				[outlineview collapseItem:item];
			else
				[outlineview expandItem:item];
