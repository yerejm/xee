...
- (XeeSaveFormatFlags)losslessSaveFlags
{
	...
	if (destcrop.origin.x < 0 || destcrop.origin.y < 0) {
		...
		if ((destcrop.origin.x < 0 && start_x != 0) || (destcrop.origin.y < 0 && start_y != 0))
			flags |= XeeNotActuallyLosslessFlag;
		else
			flags |= XeeUntransformableBlocksCanBeRetainedFlag;
	} else {
		if ((int)destcrop.origin.x % trans_mcu_width || (int)destcrop.origin.y % trans_mcu_height)
			flags |= XeeCroppingIsInexactFlag;
	}

	if ([mainimage isCropped])
		flags |= XeeNotActuallyLosslessFlag;
...
{
	if (currindex != 0)
		return [super losslessSaveTo:path flags:flags];
	...
	if (destcrop.origin.x < 0) {
		if (!(flags & XeeRetainUntransformableBlocksFlag) || start_x != 0)
			destcrop.size.width += destcrop.origin.x;
		destcrop.origin.x = 0;
	}
	if (destcrop.origin.y < 0) {
		if (!(flags & XeeRetainUntransformableBlocksFlag) || start_y != 0)
			destcrop.size.height += destcrop.origin.y;
		destcrop.origin.y = 0;
	}
	...
	@try {
		...
		for (int i = 0; i < 16; i++)
			jpeg_save_markers(&src, JPEG_APP0 + i, 0xffff);
		...
		if (!fh)
			@throw @"Couldn't open destination file";
				...
				if (offs) {
					...
					if (thumbdata) {
						...
						for (NSInteger i = 0; i < newlen; i++)
							jpeg_write_m_byte(&dest, newdata[i]);
