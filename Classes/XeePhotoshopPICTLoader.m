...
+ (BOOL)canOpenFile:(NSString *)name firstBlock:(NSData *)block attributes:(NSDictionary *)attributes;
{
	for (int offs = 0; offs <= 512; offs += 512) {
		if ([block length] > offs + 18) {
			...
			if (bytes[10] == 0x00 && bytes[11] == 0x11 && bytes[12] == 0x02 && bytes[13] == 0xff && bytes[14] == 0x0c && bytes[15] == 0x00 && bytes[16] == 0xff && bytes[17] == 0xfe)
				return YES;
		}
	}
	return NO;
	...
	for (;;) {
		...
		switch (opcode) {
			...
			case 0x0098: // packbits rectangle
			{
				...
				if (packtype != 0 || bits != 8 || comps != 1 || compsize != 8)
					[self fallback];
				...
				if ([fh readUInt16BE] != top)
					[self fallback];
				if ([fh readUInt16BE] != left)
					[self fallback];
				if ([fh readUInt16BE] != bottom)
					[self fallback];
				if ([fh readUInt16BE] != right)
					[self fallback];
				if ([fh readUInt16BE] != top)
					[self fallback];
				if ([fh readUInt16BE] != left)
			...
			case 0x009a: // directbits rectangle
			{
				...
				if (packtype != 4 || bits != 32 || comps != 3 || compsize != 8)
					[self fallback];
