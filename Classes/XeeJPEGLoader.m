...
+ (BOOL)canOpenFile:(NSString *)name firstBlock:(NSData *)block attributes:(NSDictionary *)attributes;
{
	...
	if (len >= 2 && head[0] == 0xff && head[1] == 0xd8)
		return YES;
...
- (void)load
{
	...
	for (struct jpeg_marker_struct *marker = cinfo.marker_list; marker; marker = marker->next) {
		if (marker->marker == JPEG_COM) {
			[comments addObjectsFromArray:[XeePropertyItem itemsWithLabel:@""
																textValue:[[[NSString alloc] initWithBytes:marker->data
																									length:marker->data_length
																								  encoding:NSISOLatin1StringEncoding] autorelease]]];
		} else if (XeeTestJPEGMarker(marker, 0, 5, "JFIF")) {
			[markerprops addObject:[XeePropertyItem itemWithLabel:
														NSLocalizedString(@"JFIF APP0 marker", @"JFIF APP0 marker property title")
			...
			if (marker->data_length == 12 && marker->data[11] == 2)
				invert_cmyk = YES;
		} else {
			[markerprops addObject:[XeePropertyItem itemWithLabel:[NSString stringWithFormat:
																				NSLocalizedString(@"APP%d marker", @"Unknown APPx marker property title"), marker->marker - JPEG_APP0]
	...
	if (duckyprops)
		[properties addObjectsFromArray:duckyprops];
	...
	if (!thumbonly || !thumbhandle) {
		if ([[NSUserDefaults standardUserDefaults] boolForKey:XeeJpegYUVKey] && cinfo.jpeg_color_space == JCS_YCbCr && cinfo.comp_info[0].h_samp_factor == 2 && (cinfo.comp_info[0].v_samp_factor == 2 || cinfo.comp_info[0].v_samp_factor == 1) && cinfo.comp_info[1].h_samp_factor == 1 && cinfo.comp_info[1].v_samp_factor == 1 && cinfo.comp_info[2].h_samp_factor == 1 && cinfo.comp_info[2].v_samp_factor == 1) {
			...
			for (int i = 0; i < y_rows; i++)
				y_lines[i] = y_buf + i * y_width;
			for (int i = 0; i < DCTSIZE; i++)
				cb_lines[i] = cb_buf + i * cbcr_width;
			for (int i = 0; i < DCTSIZE; i++)
				cr_lines[i] = cr_buf + i * cbcr_width;
			...
			if (cinfo.jpeg_color_space == JCS_GRAYSCALE)
				mainimage = [[[XeeBitmapImage alloc] initWithType:XeeBitmapTypeLuma8 width:width height:height] autorelease];
			else
				mainimage = [[[XeeBitmapImage alloc] initWithType:XeeBitmapTypeRGB8 width:width height:height] autorelease];
			if (!mainimage)
				XeeImageLoaderDone(NO);
	...
	if (thumbhandle) {
		...
		if (thumbnail) {
			if (correctorientation)
				[thumbnail setCorrectOrientation:correctorientation];
			[self addSubImage:thumbnail];
