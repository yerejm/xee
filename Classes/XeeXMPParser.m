...
- (id)initWithHandle:(CSHandle *)handle
{
	if (self = [super init]) {
		...
		@catch (id e) {
			NSLog(@"Error parsing XMP metadata: %@", e);
		}
...
- (NSString *)parsePropertyName:(NSXMLNode *)node
{
	...
	if ([localname isEqual:@"NativeDigest"])
		return nil; // discard meaningless NativeDigest garbage

	NSString *prefix = [node prefix];
	if (prefix) {
		if ([prefix isEqual:@"xapMM"])
			return nil; // discard horribly boring xapMM junk

		NSString *prefixname = [self reflowName:prefix capitalize:YES exceptions:prefixdict];

		if (prefixname)
			return [NSString stringWithFormat:@"%@ %@", prefixname,
											  [self reflowName:localname
													capitalize:NO
													exceptions:localnamedict]];
	}
...
- (NSString *)reflowName:(NSString *)name capitalize:(BOOL)capitalize exceptions:(NSDictionary *)exceptions
{
	...
	for (int i = 0; i < len; i++) {
		...
		if (i == 0 && capitalize)
			[newname appendFormat:@"%C", (unichar)towupper(c)];
		else
			[newname appendFormat:@"%C", (unichar)towlower(c)];
	}
...
- (NSArray *)parsePropertyValue:(NSXMLNode *)node
{
	...
	if (count == 0)
		...
		if ([name isEqual:@"rdf:Alt"] || [[child name] isEqual:@"rdf:Seq"] || [[child name] isEqual:@"rdf:Bag"]) {
			...
			if ([lis count] == 0)
				return [NSArray arrayWithObject:[child XMLString]];
			...
			while (li = [enumerator nextObject]) {
				...
				if ([children count] == 1)
					[array addObject:[self parseSingleValue:[children objectAtIndex:0]]];
				else if ([children count] > 1)
					[array addObject:[self parseSingleValue:li]];
			}

			return array;
		} else
			return [NSArray arrayWithObject:[self parseSingleValue:child]];
	} else {
	...
	if ([node kind] == NSXMLTextKind) {
		...
		if (matches) {
			...
			if ([[matches objectAtIndex:11] length]) {
				...
				if ([[matches objectAtIndex:11] isEqual:@"-"])
					timeoffs = -timeoffs;
			}
