...
{
	[(CSHandle *)png->io_ptr readBytes:(int)len toBuffer:buf];
}
	...
	if (color_type == PNG_COLOR_TYPE_RGB_ALPHA || color_type == PNG_COLOR_TYPE_GRAY_ALPHA ||
		...
		if (png_get_valid(png, info, PNG_INFO_bKGD)) {
			...
			[self setBackgroundColor:[NSColor colorWithCalibratedRed:(float)background->red / scaling
															   green:(float)background->green / scaling
																blue:(float)background->blue / scaling
															   alpha:1]];
		}
	}
...
- (SEL)startLoading
{
	...
	if (bit_depth == 16 && use16bit) {
		...
		if (hascolor) {
			if (hasalpha)
				type = XeeBitmapTypeRGBA16;
			else
				type = XeeBitmapTypeRGB16;
		} else {
			if (hasalpha)
				type = XeeBitmapTypeLumaAlpha16;
			else
				type = XeeBitmapTypeLuma16;
		}
	} else {
		if (bit_depth == 16)
	...
	if (bytesperrow < png_get_rowbytes(png, info))
...
- (SEL)loadImage
{
	...
	while (!stop) {
		...
		current_line++;
		if (current_pass == interlace_passes - 1)
			[self setCompletedRowCount:current_line];

		if (current_line >= height) {
			current_line = 0;
			current_pass++;
			if (current_pass >= interlace_passes)
...
- (SEL)finishLoading
{
	...
	for (int i = 0; i < num_comments; i++) {
		...
		if (comments[i].lang_key) {
			...
			if (langkey && [langkey length])
				key = [NSString stringWithFormat:@"%@ (%@)", key, langkey];
			text = [NSMutableString stringWithCString:comments[i].text encoding:NSUTF8StringEncoding];
		} else
			text = [NSMutableString stringWithCString:comments[i].text encoding:NSISOLatin1StringEncoding];
