...
+ (BOOL)canOpenFile:(NSString *)name firstBlock:(NSData *)block attributes:(NSDictionary *)attributes
{
	...
	if ([block length] > 6 && bytes[0] == 'G' && bytes[1] == 'I' && bytes[2] == 'F' && bytes[3] == '8' && (bytes[4] == '7' || bytes[4] == '9') && bytes[5] == 'a')
		return YES;
	return NO;
}
...
- (SEL)startLoading
{
	if (![self allocWithType:XeeBitmapTypeARGB8 width:width height:height])
		return NULL;
...
- (SEL)loadRecord
{
	...
	if (rectype == EXTENSION_RECORD_TYPE) {
		...
		if (ext) {
			if (code == GRAPHICS_EXT_FUNC_CODE && ext[0] == 4) // graphics control extension
			{
				...
				if (frametime == 0 || frametime == 1)
					frametime = 10; // oh boy, broken software!
				...
				if (ext[1] & 0x01) {
					...
					if ([frames count] == 0)
						transparent = YES;
					if (disposal == 2 && transindex == background)
						transparent = YES;
		...
		if (gif->Image.ColorMap)
			pal = [[[XeeGIFPalette alloc] initWithColorMap:gif->Image.ColorMap] autorelease];
		else
			pal = globalpal;

		XeeGIFFrame *frame = [[[XeeGIFFrame alloc] initWithWidth:framewidth
														  height:frameheight
															left:frameleft
															 top:frametop
															time:frametime
													 transparent:transindex
														disposal:disposal
		...
		if (interlaced) {
			for (int y = 0; y < frameheight; y += 8) {
				...
				if (DGifGetLine(gif, line, framewidth) == GIF_ERROR)
					return @selector(failLoading);
			}
			...
			if (DGifGetLine(gif, framedata, framewidth * frameheight) == GIF_ERROR)
				return @selector(failLoading);
		}
...
- (void)setFrame:(NSInteger)frame
{
	...
	if (frame < currframe) {
		...
		[[frames objectAtIndex:currframe] dispose:self];

	for (NSInteger i = currframe + 1; i < frame; i++)
		[[frames objectAtIndex:i] drawAndDispose:self];
...
- (void)clearImage
{
	...
	if (transindex >= 0)
		...
		val = [globalpal table][background];
...
- (void)dispose:(XeeGIFImage *)image
{
	...
	if (disposal == 2) {
		...
		if (background == transparent)
			...
			colour = [palette table][background];
