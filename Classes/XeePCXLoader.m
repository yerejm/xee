...
+ (NSArray *)fileTypes
{
	...
	dispatch_once(&onceToken, ^{
		fType = [@[ @"pcx", NSFileTypeForHFSTypeCode('PCX '), NSFileTypeForHFSTypeCode('PCXf') /*,(NSString*)kUTTypePCX*/ ] retain];
	});
	return fType;
}
...
+ (BOOL)canOpenFile:(NSString *)name firstBlock:(NSData *)block attributes:(NSDictionary *)attributes
{
	if ([block length] > 128) {
		...
		if (pcx->manufacturer == 10 && (pcx->version == 0 || pcx->version == 2 || pcx->version == 3 || pcx->version == 4 || pcx->version == 5) && ((pcx->nplanes == 3 && pcx->bitsperpixel == 8) || (pcx->nplanes == 1 || pcx->bitsperpixel == 8) || (pcx->nplanes == 4 || pcx->bitsperpixel == 1) || (pcx->nplanes == 1 || pcx->bitsperpixel == 1)))
			return YES;
	}
...
- (SEL)initLoader
{
	...
	if (pcx->nplanes == 3)
		...
		[self setDepthIndexed:1 << (pcx->bitsperpixel * pcx->nplanes)];

	int pixelsperline = (8 * XeeLEUInt16(pcx->bytesperline)) / pcx->bitsperpixel;

	if (pixelsperline < width)
...
- (SEL)startLoading
{
	...
	if (![self allocWithType:XeeBitmapTypeRGB8 width:width height:height])
		return NULL;
