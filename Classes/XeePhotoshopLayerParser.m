...
+ (NSArray *)parseLayersFromHandle:(CSHandle *)fh parentImage:(XeePhotoshopImage *)parent alphaFlag:(BOOL *)hasalpha
{
	...
	for (XeePhotoshopLayerParser *layer in layers) {
...
- (id)initWithHandle:(CSHandle *)fh parentImage:(XeePhotoshopImage *)parentimage
{
	if (self = [super init]) {
		...
		channeloffs = [[NSMutableDictionary alloc] init];
		...
		uint32_t masksize = [fh readUInt32BE];
		bytesleft -= 4;
		if (bytesleft < masksize) {
			goto outofbytes;
		}
		[fh skipBytes:masksize];
		bytesleft -= masksize;

		if (bytesleft < 4) {
			goto outofbytes;
		}
		uint32_t blendsize = [fh readUInt32BE];
		bytesleft -= 4;
		if (bytesleft < blendsize) {
			goto outofbytes;
		}
		[fh skipBytes:blendsize];
		bytesleft -= blendsize;
		...
		bytesleft -= 1;
		if (bytesleft < namelen) {
			goto outofbytes;
		}
		if (namelen) {
			layername = [[[NSString alloc] initWithData:[fh readDataOfLength:namelen] encoding:NSISOLatin1StringEncoding] autorelease];
		}
		int padbytes = ((3 - namelen) & 3);
		if (bytesleft < padbytes) {
			goto outofbytes;
		}
		[fh skipBytes:padbytes];
		bytesleft -= padbytes;
		...
		while (bytesleft >= 12) {
			...
			{
				[fh skipBytes:50];
				if ([fh readUInt16BE] != 50)
					break; // text descriptor version
				if ([fh readUInt32BE] != 16)
					break; // descriptor version
				int classlen = [fh readUInt32BE];
				[fh skipBytes:classlen * 2];
				if ([fh readUInt32BE] != 0)
					break;
				if ([fh readUInt32BE] != 'TxLr')
					break;
				...
				if ([fh readUInt32BE] != 0)
					break;
				if ([fh readUInt32BE] != 'Txt ')
					break;
				if ([fh readUInt32BE] != 'TEXT')
					break;
...
//						NSData *block=[rest subdataWithRange:NSMakeRange(offs+18,blocklen)];
	...
	outofbytes:
		...
		switch (blendmode) {
		case 'norm':
			blendname = NSLocalizedString(@"Normal", @"Photoshop layer normal blending mode name property value");
			break;
		case 'dark':
			blendname = NSLocalizedString(@"Darken", @"Photoshop layer darken blending mode name property value");
			break;
		case 'lite':
			blendname = NSLocalizedString(@"Lighten", @"Photoshop layer lighten blending mode name property value");
			break;
		case 'hue ':
			blendname = NSLocalizedString(@"Hue", @"Photoshop layer hue blending mode name property value");
			break;
		case 'sat ':
			blendname = NSLocalizedString(@"Saturation", @"Photoshop layer normal blending mode name property value");
			break;
		case 'colr':
			blendname = NSLocalizedString(@"Color", @"Photoshop layer color blending mode name property value");
			break;
		case 'lum ':
			blendname = NSLocalizedString(@"Luminosity", @"Photoshop layer luminosity blending mode name property value");
			break;
		case 'mul ':
			blendname = NSLocalizedString(@"Multiply", @"Photoshop layer multiply blending mode name property value");
			break;
		case 'scrn':
			blendname = NSLocalizedString(@"Screen", @"Photoshop layer screen blending mode name property value");
			break;
		case 'diss':
			blendname = NSLocalizedString(@"Dissolve", @"Photoshop layer dissolve blending mode name property value");
			break;
		case 'over':
			blendname = NSLocalizedString(@"Overlay", @"Photoshop layer overlay blending mode name property value");
			break;
		case 'hLit':
			blendname = NSLocalizedString(@"Hard light", @"Photoshop layer hard light blending mode name property value");
			break;
		case 'sLit':
			blendname = NSLocalizedString(@"Soft light", @"Photoshop layer soft light blending mode name property value");
			break;
		case 'diff':
			blendname = NSLocalizedString(@"Difference", @"Photoshop layer difference blending mode name property value");
			break;
		case 'smud':
			blendname = NSLocalizedString(@"Exclusion", @"Photoshop layer exclusion blending mode name property value");
			break;
		case 'div ':
			blendname = NSLocalizedString(@"Color dodge", @"Photoshop layer color dodge blending mode name property value");
			break;
		case 'idiv':
			blendname = NSLocalizedString(@"Color burn", @"Photoshop layer color burn blending mode name property value");
			break;
		default:
		...
		if (typetooltext) {
			[props addObjectsFromArray:typetooltext];
		}
		if (typetoolfonts) {
			[props addObjectsFromArray:typetoolfonts];
		}
...
- (XeeImage *)image
{
	...
	case XeePhotoshopDuotoneMode:
		...
		if (mode == XeePhotoshopGreyscaleMode) {
			[image setDepthGrey:depth alpha:hasalpha floating:depth == 32];
		} else {
			[image setDepth:[NSString stringWithFormat:NSLocalizedString(@"%d bits duotone", @"Description for duotone (Photoshop) images"), depth]
				   iconName:@"depth/rgb"];
		}
		break;
	...
	case XeePhotoshopRGBMode:
		...
		[image setDepthRGB:depth alpha:hasalpha floating:depth == 32];
		break;
	...
	[image setProperties:@[ [XeePropertyItem itemWithLabel:
...
- (CSHandle *)handleForNumberOfChannels:(int)requiredchannels
{
	...
	if (alphahandle)
		[array addObject:alphahandle];

	if ([array count] == 1) {
		return [array objectAtIndex:0];
	} else {
		return [[[XeeInterleavingHandle alloc] initWithHandles:array elementSize:depth] autorelease];
	}
}

- (CSHandle *)handleForChannel:(int)channel
{
	NSNumber *offs = [channeloffs objectForKey:@(channel)];
	if (!offs) {
		return nil;
	}
	...
	case 1:
		fh = [[handle copy] autorelease];
		[fh seekToFileOffset:start];
		return [[[XeePackbitsHandle alloc] initWithHandle:fh
													 rows:height
											  bytesPerRow:(width * depth + 7) / 8
												  channel:0
													   of:1
											 previousSize:0] autorelease];
...
- (BOOL)hasAlpha
{
	return [channeloffs objectForKey:[NSNumber numberWithInt:-1]] != nil;
}
