...
bppixel,bpcomponent,bprow,mode,alpha,flags); */
	...
	// sanity checks
	if (bppixel & 7)
		return NO; // non-byte aligned sizes not supported
	if (alpha == XeeAlphaNone && bppixel != bpcomponent * components)
		return NO;
	if (alpha != XeeAlphaNone && bppixel != bpcomponent * (components + 1))
		return NO;
	if (bprow < pixelsize * width)
		return NO;

	BOOL premult = NO;
	if (alpha == XeeAlphaPremultipliedFirst || alpha == XeeAlphaPremultipliedLast)
		premult = YES;

	BOOL trans = NO;
	if (alpha != XeeAlphaNone && alpha != XeeAlphaNoneSkipFirst && alpha != XeeAlphaNoneSkipLast)
		trans = YES;
...
{
	return XeeBitmapImageReadPixel;
}
