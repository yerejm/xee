...
static void WritePixel(XeeDreamcastImage *self, int x, int y, uint32_t col)
{
	if (x < self->width && y < self->height)
		((uint32_t *)&self->data[y * self->bytesperrow])[x] = col;
}
...
- (void)load8BitWithPalette:(BOOL)haspalette pixelFormat:(int)pixelformat
{
	...
	if (haspalette) {
		for (int i = 0; i < 256; i++)
			palette[i] = ExpandColour([fh readUInt16LE], pixelformat);
		[fh skipBytes:1024 - 256 * 2];
	} else {
		for (int i = 0; i < 256; i++)
			palette[i] = XeeMakeNRGB8(i, i, i);
	}
...
- (void)load4BitWithPalette:(BOOL)haspalette pixelFormat:(int)pixelformat
{
	...
	if (haspalette) {
		...
		for (int i = 0; i < 16; i++)
			palette[i] = XeeMakeNRGB8(i * 0x11, i * 0x11, i * 0x11);
		/*		if([self ref]) @try
		{
...
- (void)loadRectangleWithOffset:(int)offset pixelFormat:(int)pixelformat
{
	...
	for (int row = 0; row < height; row++) {
		...
		for (int col = 0; col < width; col++)
			dest[col] = ExpandColour(XeeLEUInt16(&rowbuf[2 * col]), pixelformat);
