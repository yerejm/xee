...
- (off_t)offsetInChunk
{
	return ([self offsetInFile] - curr_start);
}
...
{
	if (!curr_start)
		[NSException raise:@"XeeNoIFFChunkException" format:@"Attempted to read from an IFF handle before the first chunk has been parsed."];
	else
		[NSException raise:@"XeeReadOutsideChunkException" format:@"Attempted to read outside the current IFF chunk."];
}

- (NSString *)description
{
	return [NSString stringWithFormat:@"XeeIFFHandle for file \"%@\", position %lld in chunk %c%c%c%c",
									  path, [self offsetInChunk], (char)((curr_id >> 24) & 0xff), (char)((curr_id >> 16) & 0xff), (char)((curr_id >> 8) & 0xff), (char)(curr_id & 0xff)];
}
