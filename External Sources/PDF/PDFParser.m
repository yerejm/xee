...
- (NSString *)parsePDFStringWithParent:(PDFObjectReference *)parent
{
	...
	for (;;) {
		...
		switch (c) {
			...
			case ')':
				if (--nesting == 0) {
					return (NSString *)[[PDFString alloc] initWithData:data parent:parent parser:self];
				} else {
					b = ')';
				}
				break;
...
- (NSData *)parsePDFHexStringStartingWith:(int)c parent:(PDFObjectReference *)parent
{
	...
	for (;;) {
		...
		if (c1 == '>') {
			return (NSData *)[[PDFString alloc] initWithData:data parent:parent parser:self];
		}
		if (!IsHexDigit(c1)) {
...
- (NSString *)string
{
	...
	if (length >= 2 && bytes[0] == 0xfe && bytes[1] == 0xff) {
		return [[NSString alloc] initWithBytes:bytes + 2
										length:length - 2
