...
- (uint8_t)produceByteAtOffset:(off_t)pos
{
	if (!currbyte) {
		...
		if (err != LZWNoError)
			[NSException raise:LZWInvalidCodeException format:@"Invalid code in LZW stream (error code %d)", err];
		currbyte = LZWReverseOutputToBuffer(lzw, buffer);

		int offs = early ? 1 : 0;
		int numsymbols = LZWSymbolCount(lzw);
		if (numsymbols == 512 - offs)
			symbolsize = 10;
		else if (numsymbols == 1024 - offs)
			symbolsize = 11;
		else if (numsymbols == 2048 - offs)
			symbolsize = 12;
	}
