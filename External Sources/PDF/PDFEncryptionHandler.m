...
- (id)initWithParser:(PDFParser *)parser
{
	if (self = [super init]) {
		algorithms = nil;
		encrypt = [[parser trailerDictionary] objectForKey:@"Encrypt"];
		permanentid = [[parser permanentID] copy];
		password = nil;
		...
		if (version == 1 || version == 2) {
			int length;
			if (revision >= 3)
				length = [encrypt intValueForKey:@"Length" default:40];
			else
				length = 40;
			...
			while (key = [enumerator nextObject]) {
				...
				if ([cfm isEqual:@"V2"])
					[algorithms setObject:[[PDFRC4Algorithm alloc] initWithLength:length handler:self] forKey:key];
				else if ([cfm isEqual:@"AESV2"])
					[algorithms setObject:[[PDFAESAlgorithm alloc] initWithLength:length handler:self] forKey:key];
				else
					[NSException raise:PDFUnsupportedEncryptionException format:@"PDF encryption module \"%@\" is not supported.", cfm];
			}
...
- (BOOL)setPassword:(NSString *)newpassword
{
	...
	if (version == 1)
		...
		key = [self documentKeyOfLength:[encrypt intValueForKey:@"Length" default:40] / 8];
	else if (version == 4)
		key = [self documentKeyOfLength:16]; // This is total bullshit, but the specs don't say what to actually do for version 4.
...
- (NSData *)documentKeyOfLength:(int)length
{
	...
	if (passlength < 32) {
		...
		[md5 updateWithBytes:passbytes length:32];
	...
	if (revision >= 3)
		for (int i = 0; i < 50; i++)
			digest = [PDFMD5Engine digestForBytes:[digest bytes] length:length];
...
- (NSData *)keyOfLength:(int)length forReference:(PDFObjectReference *)ref AES:(BOOL)aes
{
	...
	if (aes)
		[md5 updateWithBytes:"sAlT" length:4];

	if (length < 11)
		return [[md5 digest] subdataWithRange:NSMakeRange(0, length + 5)];
	else
		return [md5 digest];
}
